...
for line in lines:
    # 変更箇所1: ペルソナ構築ロジック（キーワードで確実に行を特定）
    if "let personaText = settings && settings.persona ?" in line and c1 == 0:
        new_lines.append("""      let personaText = '';
      if (settings) {
          let pLines = [];
...
""")
        c1 += 1
    # 変更箇所2: UI部分（キーワードで確実に行を特定）
    elif "textarea({ className: 'input-base" in line and "例: 私はプロのWebマーケターです" in line and "value: persona" in line and c2 == 0:
        new_lines.append("""                            div({ className: 'p-4 bg-white/5 border border-white/10 rounded-xl mb-4' },
...
print(f"パッチ1(ロジック): {'✅ 成功' if c1 > 0 else '⚠️ スキップ'}")
