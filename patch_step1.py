...
# 変更箇所1: ペルソナプロンプトの構築ロジック（短くマッチさせる）
pattern1 = r"let\s+personaText\s*=\s*settings\s*&&\s*settings\.persona\s*\?[^;]+;"
...
# 変更箇所2: マイペルソナのウィザードUI化（短くマッチさせる）
pattern2 = r"textarea\(\{\s*className:\s*'input-base\s+min-h-\[\d+px\]\s+text-xs',\s*placeholder:\s*'例:\s*私はプロのWebマーケターです[^}]+\}\)"
...
print(f"パッチ2(UI): {'✅ 成功' if c2 > 0 else '⚠️ スキップ'}")
