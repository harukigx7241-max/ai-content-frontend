(function() {  
    ...
    const UserSettingModal = ({ user, onClose }) => {
        ...
        const save = () => {
            const ndb = AppDB.get();
            ndb.users[user].settings = { ...ndb.users[user].settings, keys, aiModel, theme, tone, persona, knowledge, templateUrl, notifications: notif, templates };
            AppDB.save(ndb);
            onClose();
                ...
                div({ className: 'overflow-y-auto flex-1 pr-3 hide-scrollbar space-y-6' },
                    tab === 'api' && div({ className: 'animate-in' },
                        div({ className: 'mb-6 p-4 bg-brand/10 border border-brand/20 rounded-xl leading-relaxed text-slate-300' },
                            ...
                            div({ className: 'flex flex-wrap gap-3 mt-3 text-[10px]' },
                                el('a', { href: '[https://platform.openai.com/api-keys](https://platform.openai.com/api-keys)', target: '_blank', className: 'text-brand hover:underline font-bold' }, '🔗 OpenAIキー取得'),
                                el('a', { href: '[https://aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey)', target: '_blank', className: 'text-brand hover:underline font-bold' }, '🔗 Geminiキー取得'),
                                el('a', { href: '[https://console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys)', target: '_blank', className: 'text-brand hover:underline font-bold' }, '🔗 Claudeキー取得')
                            )
                        ),
                    ...
                    tab === 'knowledge' && div({ className: 'animate-in space-y-6' },
                        ...
                        div({ className: 'pt-6 border-t border-white/10' }, 
                            ...
                            input({ className: 'input-base !py-2.5 text-sm font-mono', placeholder: '例: [https://raw.githubusercontent.com/.../templates.json](https://raw.githubusercontent.com/.../templates.json)', value: templateUrl, onChange: e => setTemplateUrl(e.target.value) }),
                            p({ className: 'text-[10px] text-slate-500 mt-2' }, '※GitHub Gist等で公開したJSONファイルのURLを指定すると、サーバーのコードを触らずに外部からテンプレートを自動同期できます。未入力の場合はローカルのファイルを参照します。')
                        )
                    ),
    ...
    const ToolCore = ({ tid, user, role, onBack }) => {  
      ...
      const fetchWebTemplates = async () => {
          ...
          try {
              const url = uData.settings.templateUrl || "/static/js/note_templates.json"; 
      ...
      const handleMagic = async (fid, extraParams = {}) => {  
        ...
        const result = await fetchWithFallback('/api/magic_generate', basePayload, magicAI, [magicAI]);
      ...
      const getFinalPrompt = () => {
          ...
          let inputs = conf.fields.map(f => `【${f.l}】\n${vals[f.id] || '（未入力）'}`).join('\n\n');
          let targetAiModel = genMode === 'api' ? aiModel : promptAiModel;
          return getAIPrefix(targetAiModel, uData.settings) + promptText + "\n\n" + inputs;
      };

      const handleCopyPrompt = () => {
          const promptText = getFinalPrompt();
          copy(promptText, 'prompt');
          showToast('📋 プロンプトをコピーしました！外部のAIに貼り付けてご利用ください。');
      };
      ...
      return div({ className: 'flex flex-col lg:flex-row h-full w-full animate-in relative lg:overflow-hidden' },  
        ...
        div({ id: 'tool-left-panel', className: 'w-full lg:w-[400px] border-b lg:border-b-0 lg:border-r border-white/10 p-6 md:p-8 shrink-0 flex flex-col lg:h-full lg:overflow-y-auto hide-scrollbar' },
          ...
          div({ className: 'mb-6 p-4 bg-white/5 border border-white/10 rounded-xl' },
              ...
              // ★ Stella Note 無限テンプレートボタン（Web連携）
              tid === 'note' && div({ className: 'mb-4 pt-4 border-t border-white/10' },
                  div({ className: 'flex justify-between items-center mb-2' },
                      h4({ className: 'text-[10px] font-bold text-slate-400' }, '🌐 Web同期 無限テンプレート (API不要)'),
                      button({ onClick: fetchWebTemplates, disabled: isLoadingWebTemplates, className: 'text-[10px] text-brand hover:text-brand-light transition' }, isLoadingWebTemplates ? '同期中...' : '🔄 最新に更新')
                  ),
                  webTemplates.length > 0 ? div({ className: 'space-y-2' },
                      webTemplates.map((temp, i) => button({
                          key: i,
                          onClick: () => {
                              setVals({...vals, ...temp.data});
                              showToast(`🎁 テンプレート「${temp.title}」をセットしました！`);
                          },
                  ...
                  ) : p({ className: 'text-[10px] text-slate-500' }, '※テンプレートが見つかりません。個人設定からWeb連携URLを登録するか、システム管理者にローカルファイル(JSON)の設定を依頼してください。')
              ),
    ...
    const AuthBox = (props) => {  
      ...
      const register = () => {
          ...
          db.users[regUsername] = {
              ...
              usage: { count: 0, tools: {} }, perms: {}, settings: { aiModel: 'chatgpt_free', keys: {openai:'', anthropic:'', google:''}, theme: 'blue', tone: '丁寧で専門的', notifications: true, persona: '', knowledge: '', templateUrl: '' },
