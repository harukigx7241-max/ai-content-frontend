...
# 1. バージョン更新
content = content.replace("const SYS_VERSION = 'v71.6.0 Ultimate Auto-Browsing Edition';", "const SYS_VERSION = 'v71.8.0 Ultimate Master Edition';")

# 2. ManualModalコンポーネントの追加
manual_modal_code = r"""    // ================================================================
    // ManualModal (総合マニュアル)
    // ================================================================
...
content = content.replace(
    ...
    "button({ onClick: () => setShowManual(true), className: 'w-full bg-brand/20 hover:bg-brand/30 border border-brand/30 py-2 rounded-lg text-xs font-bold text-brand-light transition flex items-center justify-center gap-2 mb-2' }, '📖 総合マニュアル'),\n                    button({ onClick: () => setShowSettings(true), className: 'w-full py-2 rounded-lg text-xs font-bold text-slate-300 hover:bg-white/10 transition flex items-center justify-center gap-2' }, '⚙️ 設定'),"
)

# 5. MainAppの戻り値にManualModal追加
content = content.replace(
...
with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)
print("✅ app.js に完全なマニュアル機能を追加しました！")
